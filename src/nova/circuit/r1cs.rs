...
impl<E: Engine> AllocatedRelaxedR1CSInstance<E> {
  ...
  /// Folds self with a relaxed r1cs instance and returns the result,
  /// using a pairwise verifier that absorbs both inputs in the RO.
  pub fn fold_with_r1cs_pair<CS: ConstraintSystem<<E as Engine>::Base>>(
    &self,
    mut cs: CS,
    params: &AllocatedNum<E::Base>, // hash of R1CSShape of F'
    u: &AllocatedR1CSInstance<E>,
    ...
    // absorb the running instance and the incoming instance
    self.absorb_in_ro(cs.namespace(|| "absorb U_left"), &mut ro)?;
    u.absorb_in_ro(cs.namespace(|| "absorb u"), &mut ro)?;
